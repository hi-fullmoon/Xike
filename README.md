...
回望页现已支持月历/时间流切换、注脚与关键词的本地全文搜索，以及内在天气、关键词、日期和照片组合筛选；记录详情提供带二次确认的删除操作。搜索索引与日记一起保存在 SQLCipher 加密数据库中，不会上传记录内容。

未完成的内在天气、文字、关键词和待选照片会作为加密草稿保留，应用进程重建后可以继续。设置页可按星期和本地时间开启温和提醒、夜间勿扰或暂停一周，也可开启并选择完全离线的“每日一问”题库；提醒和每日一问默认关闭，拒绝通知权限不会影响记录等核心能力。长按桌面图标可通过“记录此刻”快捷入口直接返回记录页。
