...
应用不要求账号，也没有自建服务端。日记正文、主题、搜索索引、图片和语音附件默认保存在设备本地并加密；仅当用户主动添加“此刻窗外”时，应用才会请求一次粗略位置并调用 Open-Meteo 获取当前天气。提醒、每日一问和分析均在设备本地完成。
...
- 13 个预设主题，覆盖工作、关系、身体、睡眠、兴趣等场景；旧主题在历史记录中保留，并归入对应主题的筛选与统计。
- 最多 280 字注脚。
